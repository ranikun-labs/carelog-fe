...
test('PLANNED Event entry preserves status and scheduled time through memo save', async ({
  ...
  await expect(beforeDetail).toBeVisible();
...
test('free question uses the canonical context and remains keyboard reachable', async ({
  page,
}) => {
  await useAssistantAdapter(page);
  await page.goto('/app/customers/customer-tenant-1');

  const entry = page.getByRole('button', { name: 'AI에게 물어보기', exact: true });
  await entry.focus();
  await entry.press('Enter');
  await expect(page.locator('[data-assistant-page]')).toBeVisible();

  const question = page.locator('[data-assistant-question]');
  await question.fill('다음 상담에서 확인할 내용을 알려 주세요.');
